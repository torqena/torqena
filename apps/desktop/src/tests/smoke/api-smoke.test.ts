 ...
 * Run with:
 * ```bash
 * npx vitest run tests/smoke/api-smoke.test.ts
 * ```
 *
...
const BASE_URL =
	...
	"https://torqena-api.purpleocean-69a206db.eastus.azurecontainerapps.io";
...
describe.skipIf(SKIP_SMOKE_TESTS)("Smoke: Vault Copilot Analytics API", () => {
	...
	describe("Health", () => {
		it("GET /health returns 200 with status=healthy", async () => {
			...
			expect(body.service).toBe("torqena-analytics");
		});
	});
